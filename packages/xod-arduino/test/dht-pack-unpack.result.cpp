...
namespace xod {
template <uint8_t constant_value_PORT>
struct xod_dev__dht__dht11_device {
    ...
    typedef xod_dev__dht__dht11_device::Type TypeOfDEV;
...
namespace xod {
template <typename TypeOfConstructorNodeDEV>
struct xod_dev__dht__unpack_dht11_device {

    typedef typename TypeOfConstructorNodeDEV::Type TypeOfDEV;
    ...
    xod_dev__dht__unpack_dht11_device (TypeOfOUT output_OUT) {
...
struct xod_dev__dht__dhtxx_read_raw {
    ...
    void evaluate(Context ctx) {
        static_assert(isValidDigitalPort(constant_value_PORT), "Bark!");

        State* state = getState(ctx);
        uint8_t port = constant_value_PORT;
...
typedef xod_dev__dht__unpack_dht11_device<TypeOfNode2> TypeOfNode3;
TypeOfNode3 node_3 = TypeOfNode3(0);

typedef xod_dev__dht__dhtxx_read_raw<TypeOfNode3::const_output_OUT> TypeOfNode4;
...
void runTransaction() {
    ...
    { // xod_dev__dht__dhtxx_read_raw #4
        if (g_transaction.node_4_isNodeDirty) {
            ...
            // copy data from upstream nodes into context
            ctxObj._input_PORT = node_3._output_OUT;
